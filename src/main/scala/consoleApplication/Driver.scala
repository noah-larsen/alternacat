...
object Driver extends App {
  ...
  private def lookupTargetNodes(dcfs: DCFS, sourceForestNode: Seq[String], targestForestNode: Option[Seq[String]] = None): DCFS = {
    ...
    commandInvocation.command match {
      ...
      case RemoveRelatedness => lookupTargetNodes(dcfs.withoutRelationship(sourceForest, sourceForestNode, targetForest, targestForestNode.get), sourceForestNode,
        targestForestNode)
  ...
  private def displayDescendants(dcfs: DCFS, forestLabel: String, node: Seq[String], maxDepth: Int): String = {
    Display.table(dcfs.pathsSubtree(forestLabel, node).-(node).filter(_.length - node.length <= maxDepth).toSeq.sortWith((x, y) => x.length < y.length && y.startsWith(x) || x
  ...
  private def display(path: Seq[String], withoutSubpath: Seq[String] = Nil, withForestType: Option[ForestType] = None): String = {
    val separator = " -> "
    val forestTypeToLabel = Map(
      ForestTypes.Source -> "(s)",
      ForestTypes.Target -> "(t)"
    )
    val pathString = path.drop(withoutSubpath.inits.toList.find(path.startsWith(_)).get.length).mkString(separator)
